...
static sclp_expert_encoded encode_sclp_expert(
    ...
    int64_t n_blocks = (n + QK_SCLP - 1) / QK_SCLP;
    enc.scales.resize(n_blocks);
    std::vector<float> scaled_data(n);
    for (int64_t b = 0; b < n_blocks; b++) {
        int64_t b_start = b * QK_SCLP;
        int64_t b_end = std::min(b_start + QK_SCLP, n);
        float max_val = 0.0f;
...
void llama_tensor_dequantize_sclp(
    ...
    const uint16_t * scales = (const uint16_t *)p;
    int64_t wpe = nw / ne;
    int64_t spe = (wpe + QK_SCLP - 1) / QK_SCLP;
    p += (uint64_t)ne * spe * 2;

    const uint8_t * ws = p;
    int64_t weights_per_expert = nw / ne;
    int64_t scales_per_expert = (weights_per_expert + QK_SCLP - 1) / QK_SCLP;

    for (int64_t i = 0; i < nelements; i++) {
        int64_t e = i / weights_per_expert;
        int64_t local_idx = i % weights_per_expert;
        
        int64_t scale_idx = e * scales_per_expert + (local_idx / QK_SCLP);
        float scale = bf16_to_float(scales[scale_idx]);

        uint8_t exp, smn;
        if (type == GGML_TYPE_SCLP8) {
            uint8_t b = ws[i]; // ws is ne * weights_per_expert bytes
            exp = palettes[e][b >> 4];
            smn = b & 0x0F;
        } else if (type == GGML_TYPE_SCLP4) {
            uint8_t b = ws[i/2]; // ws is ne * (weights_per_expert+1)/2 bytes
            uint8_t nibble = (i % 2 == 0) ? (b >> 4) : (b & 0xF);
            exp = palettes[e][nibble >> 2];
            smn = nibble & 0x3;
        } else { // SCLP6
        ...
        int shift = (type == GGML_TYPE_SCLP8 ? 4 : type == GGML_TYPE_SCLP4 ? 6 : 5);
        uint16_t bits = ((uint16_t)sign << 15) | ((uint16_t)exp << 7) | ((uint16_t)mant << shift);
        f32_data[i] = bf16_to_float(bits) * scale;
    }

    // Sidecar fixup
    p = ws + (uint64_t)ne * (type == GGML_TYPE_SCLP8 ? weights_per_expert : type == GGML_TYPE_SCLP4 ? (weights_per_expert + 1) / 2 : (weights_per_expert + 3) / 4 * 3);
    uint32_t sc_count;
